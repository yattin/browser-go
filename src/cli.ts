...
// Global CDP relay bridge instance
...
async function handleCDPConnection(
  req: IncomingMessage, 
  socket: Duplex, 
  head: Buffer
): Promise<void> {
  try {
    ...
    wss.handleUpgrade(req, socket, head, (ws: WebSocket) => {
      cdpRelayBridge.handleCDPConnection(ws);
    });
...
async function handleExtensionConnection(
  req: IncomingMessage, 
  socket: Duplex, 
  head: Buffer
): Promise<void> {
  try {
    ...
    const wss = new WebSocketServer({ noServer: true });
    wss.handleUpgrade(req, socket, head, (ws: WebSocket) => {
      cdpRelayBridge.handleExtensionConnection(ws);
    });
...
server.on(
  ...
  async (req: IncomingMessage, socket: Duplex, head: Buffer) => {
    ...
    if (url.pathname === '/cdp') {
      await handleCDPConnection(req, socket, head);
      return;
    }

    // Check if this is an extension connection
    if (url.pathname === '/extension') {
      await handleExtensionConnection(req, socket, head);
      return;
    }
